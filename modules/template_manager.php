...
if (!isset($_GET['id'])) {
    ...
    $sql_check_template = "SELECT id FROM templates WHERE html='' AND css='' AND user_id='$user_id' AND imgURL=''";
    ...
    if ($result_check_template && $result_check_template->num_rows > 0) {
        ...
        $sql = "INSERT INTO templates (html, css, name, user_id, reg_date, imgURL) VALUES ('', '', ?, ?, NOW(), '')";
...
$sql = "SELECT html, css, name, imgURL FROM templates WHERE id=?";
$stmt = $conn->prepare($sql);
